...
//                    var_dump(@mysqli_multi_query($mysqli, $sql02));
    ...
    public function showFriends($reqDatas) {
        ...
        try {
            //如果之前没被对方申请则新建一条记录，如果已申请则标记为好友；
            if($res = mysqli_query($mysqli,"select * from dog_request where f_tag= 1 and dog_req01 = '$uname'")){
                while($row = mysqli_fetch_assoc($res)) {
                    $resData['datas'][] = $row['dog_req02'];
                }
            }
            if($res = mysqli_query($mysqli,"select * from dog_request where f_tag= 1 and dog_req02 = '$uname'")){
                while($row = mysqli_fetch_assoc($res)) {
                    $resData['datas'][] = $row['dog_req01'];
                }
            }
